...
| `require_full_window` | **No equivalent — do not map it to `always`.** Datadog uses it to decide *whether to evaluate at all*: the monitor waits until the window holds a full set of data. KubeSense has no coverage gate, and `always` answers a different question — whether every evaluation in the window breached. Translating one to the other suppresses or delays alerts Datadog would have fired. Say so, and pick the frequency from what the monitor is actually for. |
